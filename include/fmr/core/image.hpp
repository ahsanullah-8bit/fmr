                             ...
                             bool clip = true)
{
    ...
    const int padX = static_cast<int>(std::round((resizedImageShape.width - originalImageShape.width * gain) / 2.0f));
    const int padY = static_cast<int>(std::round((resizedImageShape.height - originalImageShape.height * gain) / 2.0f));
                                   ...
                                   const float iouThreshold)
{
    ...
    const size_t num_boxes = boundingBoxes.size();
    if (num_boxes < 1)
        return {};
    ...
    for (size_t i = 0; i < sorted_indices.size(); ++i) {
        ...
        const cv::Rect& current_box = boundingBoxes[current_idx];
        ...
        for (size_t j = i + 1; j < sorted_indices.size(); ++j) {
            ...
            const cv::Rect& compare_box = boundingBoxes[compare_idx];
...
inline void draw_bbox(cv::Mat &image,
                      ...
                      float maskAlpha = 0.3f)
{
    for (const auto& prediction : predictions) {
        ...
        const cv::Point label_topleft(prediction.box.x, label_y - text_size.height - 5);
        const cv::Point label_bottomright(prediction.box.x + text_size.width + 5, label_y + baseline - 5);

        // label background
        cv::rectangle(image, label_topleft, label_bottomright, color, cv::FILLED);
